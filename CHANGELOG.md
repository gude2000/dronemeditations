...
- **iOS Chladni is now a GPU shader.** The cymatics field was a CPU SwiftUI `Canvas` filling a 140×140 grid of rectangles at 24 fps — blockier and less lively than the web. It's now a Metal fragment shader (`Chladni.metal`) applied via SwiftUI `.colorEffect`: per-pixel and sharp (with the web's tighter `mag*9` node threshold), ~30 fps, and lower CPU (frees the audio thread). Sand grains stay on a Canvas overlay. Matches the web's crispness. (Rides the next iOS build.)
