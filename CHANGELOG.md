...
- **Sharper iOS cymatics.** Tightened the Chladni node threshold to the web's `mag * 9` (was `mag * 6`) for thinner, crisper nodal lines, and bumped the render to 30 fps so vibrato reads as more active. (A full GPU/Metal shader port was attempted and reverted — it compiled but rendered no field; it needs an on-device build loop to debug, so it's deferred rather than blocking the release.)
